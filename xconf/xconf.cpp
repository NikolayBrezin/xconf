...
template<typename T> struct TreeItem : public TreeItemBase
{
    T value;
    TreeItem(const std::string& value_): value(value_)
    {
...
template<> struct TreeItem<float> : public TreeItemBase
{
    float value;
    void display() override
    {         
...
template<> struct TreeItem<uint32_t> : public TreeItemBase
{
    uint32_t value;
    void display() override
    {         
...
class ItemsFactory
{
    using TreeItemT = std::unique_ptr<TreeItemBase>;
public:
    TreeItemT create(const std::string& type, const std::string& value)
    {
        TreeItemT item;
        if (type == "float")
        {
            item = std::make_unique<TreeItem<float>>();
        }
        else if (type == "uint32_t")
        {
            item = std::make_unique<TreeItem<uint32_t>>();
        }
...
struct TreeItem
{
    ...
    void draw()
    {
        if (!_children.empty())
        {
            if (ImGui::TreeNodeEx(_key.c_str(), ImGuiTreeNodeFlags_DefaultOpen))
            {
...
void init_xconf()
{
    ...
    std::ifstream input("fadec.json.data");
    while (input)
    {
        std::string line;
